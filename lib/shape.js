 ...
 *  Pass `today` (ISO string like "2024-08-15") to derive "completed" for past sessions.
 */
export function toClientBooking(b, today = null) {
  const date = toIsoDay(b.session.date);
  let status = DB_TO_STATUS[b.status];
  // Derive "completed" for past sessions that weren't cancelled
  if (today && date < today && status !== "cancelled") {
    status = "completed";
  }
  return {
    id: b.id,
    ...
    // Raw session fields, needed to build calendar invites at the right instant.
    sessionType: b.session.type,
    startTime: b.session.startTime,
    durationMin: b.session.durationMin,
    // Manage token for email links (only included when present)
    manageToken: b.manageToken ?? null,
    // Cancellation reason (when booking is cancelled)
